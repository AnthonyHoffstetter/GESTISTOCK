...
router.get('/', async (req, res) => {
  try {
    ...
    console.error('Erreur GET /stock-out :', err.message);
    return res.status(500).json({
      ok: false,
      message: 'Erreur serveur lors du chargement des sorties.'
    });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [[bon]] = await dbPool.query(
      ...
      FROM bon b
      LEFT JOIN utilisateur u ON u.id_utilisateur = b.id_utilisateur
      WHERE b.id_bon = ? AND b.type_bon = 'OUT'
      LIMIT 1
      `,
      [id]
    );
    ...
    console.error('Erreur GET /stock-out/:id :', err.message);
    return res.status(500).json({
      ok: false,
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const nextNumber = lastBon?.reference
      ? Number(String(lastBon.reference).replace('SOR-', '')) + 1
      : 1;
    const cleanReference = `SOR-${String(nextNumber).padStart(3, '0')}`;
    ...
    for (const line of normalizedLines) {
      const [productRows] = await connection.query(
        'SELECT id_produit, prix, quantite_stock FROM produit WHERE id_produit = ? LIMIT 1',
        [line.id_produit]
      );
      ...
      if (Number(product.quantite_stock) < line.quantite) {
        await connection.rollback();
      ...
      await connection.query(
        ...
        VALUES ('OUT', ?, NOW(), ?, ?, ?)
        `,
        [line.quantite, `Sortie bon ${cleanReference}`, line.id_produit, userId]
      );
    }
    ...
    return res.status(201).json({
      ok: true,
      message: 'Bon de sortie enregistre.',
      id_bon: bonId,
      reference: cleanReference
    });
  } catch (err) {
    await connection.rollback();
    console.error('Erreur POST /stock-out :', err.message);
    return res.status(500).json({
      ok: false,
...
router.post('/:id/cancel', async (req, res) => {
  ...
  try {
    ...
    await connection.rollback();
    console.error('Erreur POST /stock-out/:id/cancel :', err.message);
    return res.status(500).json({
      ok: false,
